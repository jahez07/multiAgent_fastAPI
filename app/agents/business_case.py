...
Receives the fully enriched pipeline state from Agent 1-3 and synthesizes
...
class BusinessCase(BaseModel):
    ...
    roi_section: RoiSection
    next_steps: list[NextStep]
    key_contacts: list[str]

# System prompt

SYSTEM_PROMPT = """You are a sensor B2B sales strategist writing business cases for IoT sensor and solutions company.
...
- Concise. Executive summaries are 2 - 3 senteces max. 
- Action-oriented. Every business case ends with next steps assigned to real stakeholder roles. 

You will receive structured intelligence gathered by there previous AI agents:
...
Your job: synthesis all of this into a compelling, accurate business case in JSON format. 
...
def _summarize_stakeholders(stakeholders: list | None) -> str:
    if not stakeholders:
        return "No stakeholderrs identified"
    lines = []
...
def build_prompt(state: dict) -> str:
    """Assemble the full prompt form pipeline state."""
...
{{
    ...
    "sollution":{{
        ...
        "key_capabilities": ["3-5 feature bullets relevant to this situation"],
    ...
    "key_contact": ["List of stakeholder roles to target first, from the stakeholder map above"]
}}
...
async def write_case(state: dict) -> dict:
    """
    Agent 4 - Business Case Writer.
    Uses Claude Haiku since this is a writing task, not a reasoning task.
    """
    ...
    if result is None:
        logger.warning("Agent 4: Claude response None - storing minimal feedback")
    ...
    return {
        **state,
        "bussiness_case_sumary": json.dumps(business_case, ensure_ascii=False)
    }


def _minimal_fallback(state: dict) -> dict:
    """Return a mininal valid business case structure when Claude fails."""
    city = state.get("city") or state.get("country") or "city"
    sector = state.get("sector") or "infrastructure"

    return {
        "opportunity_title": f"{sector.replace('_', ' ').title()} opportunity in {city}",
        "executive_summary": "Analysis incomplete - Claude unavailble during writing step.",
        "problem":{
